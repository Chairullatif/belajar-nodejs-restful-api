...
const search = async (user, request) => {
    ...
    // This is for pagination, skip for skip the item, exmp: 
    // page 1 -> (page - 1) * size = 0, >> size default is 10
    // page 2 -> (page - 1) * size = 10, dst
    const skip = (request.page - 1) * request.size;

    const filters = []; 

    // wajib untuk username
    filters.push({
        username: request.username
    })

    // untuk requst optional 
    if (request.name) {
        filters.push({
            OR: [
                {
